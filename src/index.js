...
const zType = [[1, 1, 0], [0, 1, 1], [0, 0, 0]];

const sType = [[0, 2, 2], [2, 2, 0], [0, 0, 0]];

const iType = [[0, 3, 0, 0], [0, 3, 0, 0], [0, 3, 0, 0], [0, 3, 0, 0]];

const lType = [[4, 0, 0], [4, 0, 0], [4, 4, 0]];

const jType = [[0, 0, 5], [0, 0, 5], [0, 5, 5]];
...
const render = (game, block, time) => {
  ...
  if (block.isAlive) {
    ...
    insertIntoArray(block.cells, field, prevPosition.y, prevPosition.x);
    const lastBlock = block.cells
      .filter(row => !row.every(cell => !cell))
      .slice(-prevPosition.y);
...
window.onload = () => {
  const audio = new Audio('../res/theme.mp3');
